# little helper file, that contains some often-used functions
import math


def create_2d_array_increasing_numbers(x, y, starting_number=1):
    return [[(i+starting_number)+(j*x)for i in range(x)] for j in range(y)]


def print_2d_list(_2d_list):
    for line in _2d_list:
        for c in line:
            print(c, end="\t")
        print()
...
if __name__ == "__main__":
    _2d_list = create_2d_array_increasing_numbers(3, 4)
    print_2d_list(_2d_list)
