...
def perform_action_puzzle1(ship, action, value):
    if action in "NSEW":
        move_item(ship, action, value)
    elif action in "LR":
        turn_ship(ship, action, value)
    elif action == "F":
        move_item(ship, ship["direction"], value)
    else:
        print("WRONG ACTION!")

def perform_action_puzzle2(ship, waypoint, action, value):
    if action in "NSEW":
        move_item(waypoint, action, value)
    elif action in "LR":
        turn_waypoint(waypoint, action, value)
    elif action == "F":
...
def move_item(item, direction, value):
    if direction == "N":
        item["north"] += value
    elif direction == "S":
        item["north"] -= value
...
def turn_ship(ship, direction, value):
    possible_directions = "NESW"
    index = possible_directions.index(ship["direction"])
    value = value // 90
    # print(index-value)
    if direction == "L":
        ship["direction"] = possible_directions[index-value]
    elif direction == "R":
        ship["direction"] = possible_directions[(index+value)%4]

def turn_waypoint(waypoint, direction, value):
    value = value // 90
    if direction == "L":
...
# Puzzle #1
for action in data:
    perform_action_puzzle1(ship, action[0], action[1])

print(f"The answer to puzzle #1 is: {manhattan_distance(ship)}.")
...
for action in data:
    perform_action_puzzle2(ship, waypoint, action[0], action[1])

print(f"The answer to puzzle #2 is: {manhattan_distance(ship)}.")