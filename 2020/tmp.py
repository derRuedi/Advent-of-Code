...
# Solution for puzzle #2
# the worst implementation which just relies on pure luck rather than the mathematical cleverness
from random import randint
def puzzle_2_pure_luck():
    while True:
...
# startTime = datetime.now()
# print(f"The solution for puzzle #2 is {puzzle_2()}.")
# print(datetime.now() - startTime)
...
# my_puzzle = sample_data

# create useful data from input data
def create_input_data_from_data(data):
    list1 = []
    for line in data:
        dict1 = {}

        # example: "13-15 w: nwwwwwqwwwwwtww"
        min_max, letter, password = line.split(" ")
        policy_min, policy_max = min_max.split("-")
        letter = letter[0]
        ...
        dict1["policy_letter"] = letter
        dict1["policy_letter_first"] = policy_min
        dict1["policy_letter_last"] = policy_max
...
def check_passwords(data):
    data = create_input_data_from_data(data)
    valid_passwords = 0

    for my_dict in data:
        # count occurence of policy letter in password
        password = my_dict["password"]
        letter = my_dict["policy_letter"]
        policy_min = my_dict["policy_letter_first"]
        policy_max = my_dict["policy_letter_last"]

        count = password.count(letter)

        # if count within min and max, increase valid_passwords
        if policy_min <= count and count <= policy_max:
            valid_passwords += 1
    
    return valid_passwords

# print(f"The answer to puzzle #1's sample data is: {check_passwords(sample_data)}.")
# print(f"The answer to puzzle #1 is: {check_passwords(my_puzzle)}.")
...
# "dark orange bags contain 3 bright white bags, 4 muted yellow bags."
# --> dark orange : 3x white + 4x muted yellow
# bags = {
#     "dark orange": {"white": 3, "muted yellow": 4}
# }

import re


# with open("tmp2_konsti.txt", "r") as f:
...
for line in data:
    # "dark orange bags contain 3 bright white bags, 4 muted yellow bags."
    line = re.sub(" bags contain ", ":", line)
    enclosing_bag, containing = line.split(":")
    
    # "3 bright white bags, 4 muted yellow bags."
    # "1 shiny gold bag."
    # "no other bags."
    containing = re.sub("\sbags|\sbag|\.", "", containing)

    inside_bags = {}
    for item in containing.split(", "):
        if item == "no other":
            continue
        
        num, bag_name = item.split(" ", 1)
        num = int(num)
        inside_bags[bag_name] = num

    bags[enclosing_bag] = inside_bags


for enclosing_bag, inside_bags in bags.items():
    print(f"{enclosing_bag}\t: {inside_bags}")


solution = set()

def find_bag_in_bags(name, rules):
    for enclosing_bag, inside_bags in rules.items():
        # check if "name" (="shiny gold") is inside "inside_bags"
        if name in inside_bags:
            find_bag_in_bags(enclosing_bag, rules)
            solution.add(enclosing_bag)


find_bag_in_bags("shiny gold", bags)
print(solution)
print(len(solution))


def find_num_of_bags_in_bags(name, rules):
    gesamt = 0
    for bag_name, bag_count in rules[name].items():
        gesamt += bag_count * ( find_num_of_bags_in_bags(bag_name, rules) + 1 )
    return gesamt

print(find_num_of_bags_in_bags("shiny gold", bags))
