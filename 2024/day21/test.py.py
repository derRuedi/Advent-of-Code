...
with open(sys.argv[1] if len(sys.argv) > 1 else 'day21_sample.txt') as f:
...
numkeypad = ['789',
             ...
             '123',
             ' 0A']
numkeypad = {key: (x, y) for y, line in enumerate(numkeypad)
             for x, key in enumerate(line) if key != ' '}

dirkeypad = [' ^A',
             '<v>']
dirkeypad = {key: (x, y) for y, line in enumerate(dirkeypad)
             for x, key in enumerate(line) if key != ' '}
